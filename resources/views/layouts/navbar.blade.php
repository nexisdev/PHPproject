        ...
        <div class="flex gap-4 md:order-2">
            @if (\App\Helpers\ModuleHelper::isActive('kyc'))
                @if (auth()->user()->kyc?->status == 'pending')
                    <span class="btn btn-warning-alt">
                        KYC Pending
                    </span>
                @elseif(auth()->user()->kyc?->status == 'rejected')
                    <span class="btn btn-danger-alt">
                        KYC Rejected
                    </span>
                @elseif(auth()->user()->kyc?->status == 'approved')
                    <span class="btn btn-success-alt">
                        KYC Approved
                    </span>
                @else
            ...
            <div x-data x-show="$store.wallet.connected"
                class="inline-flex items-center gap-1 px-4 py-2 text-sm font-medium text-gray-500 rounded-lg bg-slate-500/20 dark:text-gray-200">
                <span x-text="$store.wallet.balance"> </span>
